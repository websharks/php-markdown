...
class MarkdownExtra extends \Michelf\Markdown {
	 ...
	 * @var string
	 */
	protected $block_tags_re = '[a-z][a-z\-0-9]*\-div(?!\-)|p|div|h[1-6]|blockquote|pre|table|dl|ol|ul|details|summary|address|form|fieldset|iframe|hr|legend|article|section|nav|aside|hgroup|header|footer|figcaption|figure';

	/**
	 * Tags treated as block tags only if the opening tag is alone on its line.
	 * @var string
	 */
	protected $context_block_tags_re = '[a-z][a-z\-0-9]*\-(?!div)[a-z0-9]+(?!\-)|script|noscript|style|ins|del|iframe|object|source|track|param|math|svg|canvas|audio|video';
	...
	/**
	 * Replace ~~strike~~ with <del>strike</del>.
	 * @param  string $text
	 * @return string
	 */
	protected function doStrikethrough($text) {
		return preg_replace_callback(
			'/(~{2}(?!\s))([^~]*[^\s~])(~{2})/',
			array($this, '_doStrikethrough_callback'),
			$text
		);
	}

	/**
	 * Callback for replacing ~~strike~~ with <del>strike</del>.
	 * @param  array $matches
	 * @return string
	 */
	protected function _doStrikethrough_callback($matches) {
		return '<del>'.$matches[2].'</del>';
	}
